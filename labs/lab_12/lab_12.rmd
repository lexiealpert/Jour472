...
I think the top ten words show many similiarities, for instance clients and client
...
unique_words <- unique_words |> anti_join(stop_words) |>
  group_by(word) |>
  tally(sort=TRUE) |>
...
In 2021 there were 37, which was the highest. The lowest were 2011, and 2023. these numbers all got much higher after 2017. One thing to try and clarity the reasons could be to look at longer sentences like three words or four for more context. Another thing that could be done to gain more clarification would be to eliminate more repetitive words.

```{r}
trigram_count <- md_attorney_sanctions |>
  filter(str_detect(text, "attorney trust")) |>
  group_by(fiscal_year) |>
  summarise(trigram_occurrences = n()) |>
  arrange(desc(trigram_occurrences))
