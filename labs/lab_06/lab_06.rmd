...
A2. August is the worst month, because it is warmer months
...
maryland_zcta |> summarise(avg_pct_rent = mean(pct_rent))
```
...
A4: How many different neighborhoods had 911 calls with over 40 percent homeownership?

```{r}
baltimore_911_with_demographics |>
  filter(pct_homeowner >=40) |>
  group_by(zip_code, population) |>
  summarize(calls= n()) |>
  mutate(rate = (calls/population)*1000) |>
  arrange(desc(rate))
