...
county_outages_march <- read_csv("data/county_outages_march2023.csv")  |> clean_names()

county_outages_march <- county_outages_march |>
...
merged_outages <- county_outages_march |>
  left_join(md_counties, by = c("county" = "county"))
```

```{r}
#Map it out
ggplot() +
   geom_sf(data = county_outages_march, aes(fill=pct_difference)) +
  theme_minimal() +
  scale_fill_viridis_b()
...
#everytime I put something here they do not join and im not sure what im doing wrong. 
