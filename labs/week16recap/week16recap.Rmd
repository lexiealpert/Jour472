...
#### \* im not sure exactly what im doing wrong but I cannot get the bars to load even though the graph is coming up.

```{r}
# Calculate the percentage of overdue inspections per county
overdue_childcare<- childcare_with_counties |>
  group_by(COUNTYFP) |>
  summarise(percentage_overdue = mean(overdue) * 100)

# Create the chart with ggplot
overdue_childcare |>
  ggplot()+
  labs(title = "Percentage of Overdue Childcare Facility Inspections per County",
       x = "County",
...
```{r}
# Join the dataframes
childcare_with_counties<- merge(childcare_with_counties, md, by = "GEOID", all.x = TRUE)
# everytime I did an inner join or a left join i got an error so I googled how to merge the data together but I realize I dont think this is the write information. 

# Calculate the rates for children per facility
childcare_with_counties <- childcare_with_counties |>
  mutate(children_per_facility = total / children_pop) 
...
The far left and far right are lacking facilities, along with places in the middle

```{r}
ggplot() +
  geom_sf(data = childcare_with_counties, aes(fill = children_per_facility))
```
...
median_income_md <- get_acs(geography = 'county',
...
median_income_md <- median_income_md |>
...
```{r}
# Join the dataframes
childcare_with_counties<- merge(childcare_with_counties, median_income_md, by = "GEOID", all.x = TRUE)
# everytime I did an inner join or a left join i got an error so I googled how to merge the data together but I realize I dont think this is the write information. 
```

```{r}
# Create a scatterplot with median income and rate of children per facility
ggplot(childcare_with_counties, aes(x = median_income, y = children_per_facility)) +
  geom_point() +
  labs(title = "median income and rate of children per facility",
       x = "Median Income",
       y = "Rate of Children per Facility")

```

```{r}
# Create a correlation test.
cor.test(childcare_with_counties$median_income, childcare_with_counties$children_per_facility)

```

##### A5: I think that the number is 0.003 which is extremely low which indicates that this is showing no coorelation which is also shown on the scatter plot with it being all over the place
...
data("stop_words")
unique_words <- reports |> anti_join(stop_words) |>
  #i had trouble loading the stop words without getting an error sign 
