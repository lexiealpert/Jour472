...
URL: <https://datawrapper.dwcdn.net/0fexH/1/>
...
**A3.** <https://datawrapper.dwcdn.net/1Qw9G/1/>

```{r}
montgomery_2022_overdoses <- read_csv("data/montgomery_2022_overdoses.csv") |> clean_names()

moco_overdoses_byzip <- montgomery_2022_overdoses  |>
  group_by(zip) |>
  summarize(
  total_calls = n()) |>
mutate(pct_of_total_calls = (total_calls / sum(total_calls)) * 100)

write_csv(moco_overdoses_byzip, 'data.moco_overdoses_byzip.csv')

#data wrpper has base maps , cloropleth map and then there should be zip codes for usa maryland 
```
